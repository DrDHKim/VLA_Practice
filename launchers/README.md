...
- `06_데이터수집.command`: CARLA dataset을 여러 scene으로 수집한다. `SCENE_COUNT`, scene별 초, FPS, 해상도, route length, weather, output root를 파일 상단에서 조정한다. 각 scene은 `scene_000`, `scene_001` 형식으로 저장되고 전체 metadata는 output root의 `metadata.jsonl`로 합쳐진다.
